...
class DuelMixLoss(nn.Module):
    def __init__(self, model, target_model, mixer, target_mixer,
                 ...
                 focus_config=None, occupancy_model=None):
    ...
    def forward(self, rewards, actions, prev_actions, terminated, mask,
        ...
        if invalid_current.any():
            raise RuntimeError(
                "DuelMIX Focus received a live transition with no valid current actions. "
...
class DuelMixTorchPolicy(Policy):
    ...
    def __init__(self, obs_space, action_space, config):
        ...
        config = dict(ray.rllib.agents.duelmix_focus.duelmix.DEFAULT_CONFIG, **config)
        ...
        self.loss = DuelMixLoss(
            ...
            focus_config, self.occupancy_model,
        )
