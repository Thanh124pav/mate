"""QPLEX + Shared Encoder (SE) Trainer."""
...
DEFAULT_CONFIG = with_common_config({
    ...
    # === Shared Encoder ===
    "shared_encoder": {
        "n_targets": 8,          # number of targets (auto-updated by train.py)
        "pred_hidden": 128,      # prediction head hidden dim
        "aux_loss_weight": 0.1,  # MSE auxiliary loss coefficient
    },
...
class QPlexSETrainer(SimpleQTrainer):
    @classmethod
    ...
    def validate_config(self, config: TrainerConfigDict) -> None:
        ...
        if config["framework"] != "torch":
            raise ValueError("Only framework=torch supported for QPlexSETrainer!")

    @override(SimpleQTrainer)
    def get_default_policy_class(self, config: TrainerConfigDict) -> Type[Policy]:
        return QPLEXSETorchPolicy
