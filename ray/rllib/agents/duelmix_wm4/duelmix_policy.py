"""DuelMix + Dreamer-style World Model (WM4) Policy."""
...
from ray.rllib.agents.duelmix_wm4.world_model_v4 import LatentWorldModel
...
class DuelMixWM4Loss(nn.Module):
    def __init__(
        self,
        model, target_model,
        mixer, target_mixer,
    ...
    def forward(
        ...
        if self.use_imagination_targets:
            ...
            for h in range(H):
                ...
                with torch.no_grad():
                    imag_obs = self.world_model.obs_decoder(imag_feature)

                if h == 0:
                    step_actions = act_flat
                else:
                    ...
                    with torch.no_grad():
                        v_imag, a_imag, _ = _mac_duelmix(self.model, obs_aug_imag, h_zero)
                    q_imag = v_imag + a_imag
                    step_actions = q_imag.argmax(dim=-1)
            ...
            first_imag_state = imag_decoded_states[0]
            imag_recon = ((first_imag_state - real_next_state) ** 2).mean(dim=-1)
            imag_recon_loss = imag_recon.reshape(B, T)
            imag_recon_loss = (imag_recon_loss * wm_mask).sum() / wm_mask.sum().clamp(min=1)
        ...
        total_loss = td_loss + self.wm_loss_weight * wm_loss + self.imagination_loss_weight * imag_recon_loss
...
class DuelMixWM4TorchPolicy(Policy):
