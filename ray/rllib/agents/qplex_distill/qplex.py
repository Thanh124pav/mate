...
from ray.rllib.agents.qplex_v2.qplex import DEFAULT_CONFIG as _QPLEX_V2_DEFAULT
...
class QPlexDistillTrainer(SimpleQTrainer):
    @classmethod
    ...
    def validate_config(self, config: TrainerConfigDict) -> None:
        ...
        if config["framework"] != "torch":
            raise ValueError("Only framework=torch is supported for QPlexDistillTrainer.")

    @override(SimpleQTrainer)
    def get_default_policy_class(self, config: TrainerConfigDict) -> Type[Policy]:
        return QPLEXDistillTorchPolicy
