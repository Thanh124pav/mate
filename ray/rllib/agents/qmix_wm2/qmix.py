...
from ray.rllib.agents.qmix_wm2.qmix_policy import QMixWM2TorchPolicy
from ray.rllib.evaluation.worker_set import WorkerSet
from ray.rllib.execution.concurrency_ops import Concurrently
from ray.rllib.execution.metric_ops import StandardMetricsReporting
from ray.rllib.execution.replay_ops import Replay, SimpleReplayBuffer, StoreToReplayBuffer
...
class QMixWM2Trainer(SimpleQTrainer):
    @classmethod
    ...
    def validate_config(self, config: TrainerConfigDict) -> None:
        ...
        if config["framework"] != "torch":
            raise ValueError("Only framework=torch supported for QMixWM2Trainer!")

    @override(SimpleQTrainer)
    def get_default_policy_class(self, config: TrainerConfigDict) -> Type[Policy]:
        return QMixWM2TorchPolicy

    @staticmethod
    @override(SimpleQTrainer)
    def execution_plan(workers: WorkerSet, config: TrainerConfigDict, **kwargs) -> LocalIterator[dict]:
        assert len(kwargs) == 0, "QMIX_WM2 execution_plan does NOT take additional parameters"
