"""QPLEX + RSSM World Model (WM2) + Shared Encoder (SE) Trainer."""
...
DEFAULT_CONFIG = with_common_config({
    ...
    # === World Model V2 (RSSM) — same as qplex_wm2 ===
    "world_model_v2": {
        "stoch_dim": 32,
        "deter_dim": 128,
        "hidden_dim": 128,
    ...
    # === Shared Encoder (SE) — same as qplex_se ===
    "shared_encoder": {
...
class QPlexWM2SETrainer(SimpleQTrainer):
    @classmethod
    ...
    def validate_config(self, config: TrainerConfigDict) -> None:
        ...
        if config["framework"] != "torch":
            raise ValueError("Only framework=torch supported for QPlexWM2SETrainer!")

    @override(SimpleQTrainer)
    def get_default_policy_class(self, config: TrainerConfigDict) -> Type[Policy]:
        return QPLEXWM2SETorchPolicy
