"""QPLEX + Dreamer-style World Model (WM2) Policy.

True world model that conditions on actions:
...
Training flow:
  ...
  3. Imagination rollout — H steps forward in latent space for multi-step targets
  4. QPLEX loss — obs/state augmented with latent features

Metrics logged to wandb:
  - wm_recon_loss, wm_reward_loss, wm_kl_loss, wm_total_loss
  - reward_bonus_mean, td_loss, loss
"""
...
class QPLEXWM2Loss(nn.Module):
    """QPLEX loss with Dreamer-style RSSM world model.
    ...
    World model provides:
      ...
      - Imagination rollout for multi-step TD targets (optional)
      - Reward shaping from predicted target proximity
    """
    ...
    def _compute_reward_bonus(self, state_decoded, state_real):
        ...
        Low reconstruction error → world model understands dynamics → bonus.

        Args:
            state_decoded: [B, T, state_dim]
            state_real: [B, T, state_dim]
    ...
    def forward(
        ...
        # =================================================================
        with torch.no_grad():
            # Use EMA world model for augmentation (changes slowly → stable)
            _, ema_features, _ = self.ema_world_model.compute_loss(
                obs, actions, state, rewards, wm_mask
            )
            feature_det = ema_features  # already no grad
...
class QPLEXWM2TorchPolicy(Policy):
    """QPLEX + RSSM World Model policy.

    The RNN model receives obs augmented with RSSM latent features (feature_dim).
    This gives each agent a compressed representation of the full predicted environment state.
    """
    ...
    def __init__(self, obs_space, action_space, config):
        ...
        # =====================================================================
        # World Model (RSSM) — create FIRST for feature_dim
        # =====================================================================
        ...
        feature_dim = self.world_model.feature_dim  # stoch + deter

        # EMA copy of world model (slow-moving, for stable augmentation)
        import copy
        self.ema_world_model = copy.deepcopy(self.world_model)
        for p in self.ema_world_model.parameters():
            p.requires_grad = False  # EMA model never trains directly
    ...
    def _augment_obs_inference(self, obs_tensor):
        """Augment obs with EMA world model latent features at inference time (stable)."""
        B = obs_tensor.shape[0]
        feature = self.ema_world_model.encode_obs(obs_tensor)  # [B, feature_dim]
    ...
    def learn_on_batch(self, samples):
        ...
        # EMA update: slow-moving world model for stable augmentation
