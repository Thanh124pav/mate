...
import ray
from .mixers import DuelMixerV2
...
class QPLEXLoss(nn.Module):
    ...
    def forward(
        ...
        if self.mixer is not None:
            ...
            ans_adv = self.mixer(chosen_action_qvals, state, actions_onehot, max_action_vals = max_action_vals, is_v = False)
...
class QPLEXTorchPolicy(Policy):
    ...
    def __init__(self, obs_space, action_space, config):
        ...
        config = dict(ray.rllib.agents.qplex_v2.qplex.DEFAULT_CONFIG, **config)
        ...
        self.mixer = DuelMixerV2(self.args, self.n_agents, self.n_actions, self.env_global_state_shape, config['mixing_embed_dim'], self.args.ffn_hidden_dim, self.args.num_kernel).to(self.device)
        self.target_mixer = DuelMixerV2(self.args, self.n_agents, self.n_actions, self.env_global_state_shape, config['mixing_embed_dim'], self.args.ffn_hidden_dim, self.args.num_kernel).to(self.device)
        assert config['mixer'] == 'qplex_v2', f"Expected qplex_v2, get {config['mixer']}"
