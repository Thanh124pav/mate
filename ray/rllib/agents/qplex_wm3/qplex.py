"""QPLEX_WM3: Distributional QPLEX + Stochastic World Model Trainer."""
...
DEFAULT_CONFIG = with_common_config({
    ...
    # === WM3: Distributional Q + Stochastic World Model ===
    "wm3": {
        "n_targets": 8,              # auto-updated by train.py
        "n_quantiles": 8,            # quantile points for distributional Q
        "n_modes": 3,                # Gaussian mixture modes per target
        "pred_hidden": 128,          # prediction head hidden dim
        "aux_loss_weight": 0.1,      # NLL auxiliary loss coefficient
        "coverage_bonus_coeff": 0.05, # probability region coverage reward bonus
    },
...
class QPlexWM3Trainer(SimpleQTrainer):
    @classmethod
    ...
    def validate_config(self, config: TrainerConfigDict) -> None:
        ...
        if config["framework"] != "torch":
            raise ValueError("Only framework=torch supported for QPlexWM3Trainer!")

    @override(SimpleQTrainer)
    def get_default_policy_class(self, config: TrainerConfigDict) -> Type[Policy]:
        return QPLEXWM3TorchPolicy
