...
class QPLEXTorchPolicy(Policy):
    ...
    def __init__(self, obs_space, action_space, config):
        ...
        config = dict(ray.rllib.agents.qplex.qplex.DEFAULT_CONFIG, **config)
        ...
        self.mixer = DuelMixer(self.args, self.n_agents, self.n_actions, self.env_global_state_shape, config['mixing_embed_dim'], self.args.ffn_hidden_dim, self.args.num_kernel).to(self.device)
        self.target_mixer = DuelMixer(self.args, self.n_agents, self.n_actions, self.env_global_state_shape, config['mixing_embed_dim'], self.args.ffn_hidden_dim, self.args.num_kernel).to(self.device)
        assert config['mixer'] == 'qplex', f"Expected qplex, get {config['mixer']}"
