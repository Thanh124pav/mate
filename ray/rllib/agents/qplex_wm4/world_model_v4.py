"""Dreamer-style RSSM World Model for Multi-Agent QPLEX (WM4).
...
Architecture (RSSM — Recurrent State-Space Model):
  ...
  - ObsDecoder: latent → per-agent observations (for Dreamer policy-in-loop)
...
class LatentWorldModel(nn.Module):
    """Complete RSSM-based world model for multi-agent QPLEX (WM4).

    Combines: ObservationEncoder + ActionEmbedding + TransitionModel +
              StateDecoder + RewardPredictor + ObsDecoder
    ...
    def compute_loss(self, obs, actions, state, rewards, mask, return_posteriors=False):
        """Compute world model losses: reconstruction + reward + KL + obs reconstruction.
