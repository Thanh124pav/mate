"""SPECTra + RSSM World Model (WM2) Policy."""
...
import ray
from ray.rllib.agents.spectra_wm2.mixers import SPECTraMixer
from ray.rllib.agents.spectra_wm2.model import SPECTraRNNModel, _get_size
...
class SPECTraWM2Loss(nn.Module):
    def __init__(
        self,
        model,
        target_model,
...
class SPECTraWM2TorchPolicy(Policy):
    """SPECTra with RSSM world-model latent features."""

    def __init__(self, obs_space, action_space, config):
        _validate(obs_space, action_space)
        config = dict(ray.rllib.agents.spectra_wm2.spectra.DEFAULT_CONFIG, **config)
        ...
        if config["mixer"] is None:
            ...
            self.mixer = SPECTraMixer(
                self.n_agents, augmented_state_shape, mixing_dim, embed_dim, n_heads
            ).to(self.device)
...
def _validate(obs_space, action_space):
    ...
    if not isinstance(action_space.spaces[0], Discrete):
        raise ValueError(f"SPECTRA_WM2 requires discrete action space, got {action_space.spaces[0]}")
