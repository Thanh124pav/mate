...
class SPECTraWM2Trainer(SimpleQTrainer):
    @classmethod
    ...
    def validate_config(self, config: TrainerConfigDict) -> None:
        ...
        if config["framework"] != "torch":
            raise ValueError("Only `framework=torch` supported for SPECTraWM2Trainer!")

    @override(SimpleQTrainer)
    def get_default_policy_class(self, config: TrainerConfigDict) -> Type[Policy]:
        return SPECTraWM2TorchPolicy
    ...
    def execution_plan(
        ...
        assert len(kwargs) == 0, "SPECTRA_WM2 execution_plan does NOT take additional parameters"
