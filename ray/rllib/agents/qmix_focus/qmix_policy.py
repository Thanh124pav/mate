...
import ray
from ray.rllib.agents.qmix_focus.mixers import VDNMixer, QMixer
...
class QMixLoss(nn.Module):
    ...
    def forward(
        ...
        # action shape: [B, T, n_agents]
        # actions.unsqueeze(3): [B, T, n_agents, 1]
        ...
        target_mac_out = _unroll_mac(self.target_model, next_obs) # # [B, T, n_agents, n_actions]
...
class QMixTorchPolicy(Policy):
    ...
    def __init__(self, obs_space, action_space, config):
        ...
        config = dict(ray.rllib.agents.qmix_focus.qmix.DEFAULT_CONFIG, **config)
        self.framework = "torch"
        super().__init__(obs_space, action_space, config)
        self.n_agents = len(obs_space.original_space.spaces)
        config["model"]["n_agents"] = self.n_agents
        self.n_actions = action_space.spaces[0].n # 25  for default config
    ...
    def compute_actions(
        ...
        **kwargs
        ...
        with torch.no_grad():
            ...
            # Use a large timestep value if timestep is None (e.g., during inference)
            # This ensures we use the final epsilon value from the schedule
            if timestep is None:
                timestep = int(1e9)
