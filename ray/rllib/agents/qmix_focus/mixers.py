...
class VDNMixer(nn.Module):
    ...
    def credit_weights(self, agent_qs, states):
        return torch.ones_like(agent_qs)
...
class QMixer(nn.Module):
    ...
    def forward(self, agent_qs, states):
        ...
        agent_qs = agent_qs.view(-1, 1, self.n_agents) # [B * T , 1, n_agents]
        # First layer
        ...
        y = torch.bmm(hidden, w_final) + v # [B*T, 1, 1]
        # Reshape and return
        q_tot = y.view(bs, -1, 1) # [B, T, 1]
        return q_tot
    ...
    def credit_weights(self, agent_qs, states):
        ...
        weights = torch.bmm(w1, w_final).squeeze(-1) + 1e-10
        return weights.view(bs, -1, self.n_agents)
