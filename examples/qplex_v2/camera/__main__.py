...
r"""
Example of QPLEX agents for the Multi-Agent Tracking Environment.

.. code:: bash

    python3 -m examples.qplex.camera

    python3 -m mate.evaluate --episodes 1 --render-communication \
        --camera-agent examples.qplex:QPLEXV2CameraAgent \
        --camera-kwargs '{ "checkpoint_path": "examples/qplex/camera/ray_results/QPLEX/latest-checkpoint" }'
"""
...
import mate
from examples.qplex_v2.camera.agent import QPLEXCameraAgent
from examples.qplex_v2.camera.train import experiment
...
def main():
    ...
    if not os.path.exists(args.checkpoint_path):
        print(
            (
                ...
                f'  python -m examples.qplex.camera.train'
            ),
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    # Make agents ##############################################################
    camera_agent = QPLEXCameraAgent(checkpoint_path=args.checkpoint_path)
