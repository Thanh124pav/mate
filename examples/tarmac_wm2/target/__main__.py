...
.. code:: bash
    ...
    python3 -m examples.tarmac_wm2.target

    python3 -m mate.evaluate --episodes 1 --render-communication \
        --target-agent examples.tarmac_wm2:TarMACWM2TargetAgent \
...
import mate
from examples.tarmac_wm2.target.agent import TarMACWM2TargetAgent
from examples.tarmac_wm2.target.train import experiment
...
def main():
    ...
    if not os.path.exists(args.checkpoint_path):
        print(
            (
                ...
                f'  python -m examples.tarmac_wm2.target.train'
            ),
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    target_agent = TarMACWM2TargetAgent(checkpoint_path=args.checkpoint_path)
