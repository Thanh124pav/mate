...
.. code:: bash
    ...
    python3 -m examples.qmix_wm2.target

    python3 -m mate.evaluate --episodes 1 --render-communication \
        --target-agent examples.qmix_wm2:QMIXWM2TargetAgent \
...
import mate
from examples.qmix_wm2.target.agent import QMIXWM2TargetAgent
from examples.qmix_wm2.target.train import experiment
...
def main():
    ...
    if not os.path.exists(args.checkpoint_path):
        print(
            (
                ...
                f'  python -m examples.qmix_wm2.target.train'
            ),
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    target_agent = QMIXWM2TargetAgent(checkpoint_path=args.checkpoint_path)
