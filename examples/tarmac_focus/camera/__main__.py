...
r"""
Example of TarMAC_FOCUS agents for the Multi-Agent Tracking Environment.

.. code:: bash

    python3 -m examples.tarmac_focus.camera

    python3 -m mate.evaluate --episodes 1 --render-communication \
        --camera-agent examples.tarmac_focus:TarMACFocusCameraAgent \
        --camera-kwargs '{ "checkpoint_path": "examples/tarmac_focus/camera/ray_results/TarMAC_FOCUS/latest-checkpoint" }'
"""
...
import mate
from examples.tarmac_focus.camera.agent import TarMACFocusCameraAgent
from examples.tarmac_focus.camera.train import experiment
...
def main():
    ...
    if not os.path.exists(args.checkpoint_path):
        print(
            (
                ...
                f"  python -m examples.tarmac_focus.camera.train"
            ),
            file=sys.stderr,
        )
        sys.exit(1)

    camera_agent = TarMACFocusCameraAgent(checkpoint_path=args.checkpoint_path)
    ...
    for _ in range(MAX_EPISODE_STEPS):
        ...
        camera_joint_observation, _, done, camera_infos = env.step(camera_joint_action)
        env.render()
        if done:
            break
