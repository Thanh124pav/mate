...
.. code:: bash
    ...
    python3 -m examples.mappo_wm2.target

    python3 -m mate.evaluate --episodes 1 --render-communication \
        --target-agent examples.mappo_wm2:MAPPOWM2TargetAgent \
...
import mate
from examples.mappo_wm2.target.agent import MAPPOWM2TargetAgent
from examples.mappo_wm2.target.train import experiment
...
def main():
    ...
    if not os.path.exists(args.checkpoint_path):
        print(
            (
                ...
                f'  python -m examples.mappo_wm2.target.train'
            ),
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    target_agent = MAPPOWM2TargetAgent(checkpoint_path=args.checkpoint_path)
