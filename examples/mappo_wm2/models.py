...
class MAPPOWM2Model(TorchRNN, nn.Module):
    def __init__(
        self,
        obs_space,
        action_space,
        ...
        **kwargs,
        ...
        if self.wm2_feature_dim > 0 and n_wm2_actions is None:
            raise ValueError('MAPPOWM2Model requires a discrete action space for WM2.')
        ...
        self.critic = SimpleRNN(
            name='critic',
            input_dim=self.global_state_dim + self.wm2_feature_dim,
    ...
    def forward_rnn(self, inputs, state, seq_lens):
        ...
        actor_input = torch.cat((local_obs, wm2_feature), dim=-1)
        ...
        critic_input = torch.cat((global_state, wm2_feature), dim=-1)
    ...
    def _wm2_auxiliary_loss(self, obs, loss_inputs, reference_loss):
        return wm2_auxiliary_loss(
            self.world_model_v2,
            loss_inputs,
            reference_loss,
            self.flat_obs_dim,
    ...
    def custom_loss(self, policy_loss, loss_inputs):
        ...
        wm2_loss, wm2_stats = self._wm2_auxiliary_loss(obs, loss_inputs, reference_loss)
        self._focus_stats = {
        ...
        self._focus_stats.update({f'wm2/{key}': value for key, value in wm2_stats.items()})
...
ModelCatalog.register_custom_model('MAPPOWM2Model', MAPPOWM2Model)
