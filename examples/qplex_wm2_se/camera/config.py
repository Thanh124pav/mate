"""QPLEX_WM2_SE (pure) — RSSM World Model + Shared Encoder position prediction.

WM2 augments obs/state with RSSM latent features.
SE adds a PredictionHead on RNN hidden states with MSE position prediction loss.
...
from ray.rllib.agents.qplex_wm2_se import DEFAULT_CONFIG
...
tune.register_env('mate-qplex_wm2_se.camera', make_env)

from ray.rllib.agents.qplex_wm2_se.qplex import QPlexWM2SETrainer
tune.register_trainable('QPLEX_WM2_SE', QPlexWM2SETrainer)

config = {
    **DEFAULT_CONFIG,
    'framework': 'torch',
    'seed': 0,
    'env': 'mate-qplex_wm2_se.camera',
    'env_config': {
    ...
    'shared_encoder': {
        'n_targets': 8,      # auto-updated by train.py based on env yaml
        'pred_hidden': 128,
        'aux_loss_weight': 0.1,
    },
    'gamma': 0.99,
