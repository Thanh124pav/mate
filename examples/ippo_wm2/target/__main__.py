...
.. code:: bash
    ...
    python3 -m examples.ippo_wm2.target

    python3 -m mate.evaluate --episodes 1 --render-communication \
        --target-agent examples.ippo_wm2:IPPOWM2TargetAgent \
...
import mate
from examples.ippo_wm2.target.agent import IPPOWM2TargetAgent
from examples.ippo_wm2.target.train import experiment
...
def main():
    ...
    if not os.path.exists(args.checkpoint_path):
        print(
            (
                ...
                f'  python -m examples.ippo_wm2.target.train'
            ),
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    target_agent = IPPOWM2TargetAgent(checkpoint_path=args.checkpoint_path)
