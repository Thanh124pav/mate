...
r"""Demo/eval script for HiTMAC Phase 2A — QPLEX_V2 Coordinator + QPLEX Executor.

Train Phase 1 (QPLEX executors):
    python -m examples.hitmac.camera.train --env MATE-4v8-9.yaml

Train Phase 2A QPLEX_V2 (QPLEX_V2 coordinator):
...
Eval:
    python -m examples.hitmac.coordinator.qplex_v2.camera \
        --checkpoint-path examples/hitmac/coordinator/qplex_v2/camera/ray_results/HiTMAC-Coordinator-QPLEX_V2/latest-checkpoint \
...
import mate
from examples.hitmac.coordinator.qplex_v2.camera.agent import HiTMACQPLEXV2CoordinatorCameraAgent
from examples.hitmac.coordinator.qplex_v2.camera.train import experiment
...
def main():
    ...
    parser.add_argument(
        ...
        help='path to QPLEX_V2 coordinator checkpoint (Phase 2A output)',
    )
    parser.add_argument(
    ...
    if not os.path.exists(args.checkpoint_path):
        print(
            f'QPLEX_V2 coordinator checkpoint ("{args.checkpoint_path}") does not exist.\n'
            f'Run the following to train first:\n'
            f'  python -m examples.hitmac.coordinator.qplex_v2.camera.train '
    ...
    # Make agent
    camera_agent = HiTMACQPLEXV2CoordinatorCameraAgent(
    ...
    print(f'Coordinator: QPLEX_V2 | Executor: {exec_label}')
