#!/usr/bin/env python3
"""HiTMAC Phase 2A — Train MAPPO coordinator với fixed Phase 1 QPLEX executor.

Theo paper HiTMAC (NeurIPS 2020, Section 3.3 "Training Strategy"):

  Phase 1 (cần train trước):
      python -m examples.hitmac.camera.train --env MATE-4v8-9.yaml

  Phase 2A (ĐÂY — train coordinator):
...
from examples.hitmac.coordinator.mappo.camera.config import config
...
def train(
    ...
    if WandbLoggerCallback.is_available():
        ...
        group = group or f'hitmac.coordinator.mappo.camera.{experiment.name}'
    ...
    # Update train batch size theo num_workers
    if num_workers > 0:
...
def main():
    ...
    parser.add_argument(
        ...
        help='path to coordinator checkpoint to restore from',
    )
    parser.add_argument(
