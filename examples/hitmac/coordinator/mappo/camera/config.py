"""HiTMAC Phase 2A — Config train MAPPO coordinator với fixed QPLEX executor.

Wrapper chain (Phase 2A):
    mate.DiscreteCamera(levels=5)       → action: Discrete(25) [cho executor bên trong]
    mate.MultiCamera
    ...
    HiTMACCoordinatorWrapper(executor_checkpoint)
        # obs_space:    base obs (D dims) — không có task bits
        # action_space: MultiDiscrete((2,)*num_targets) per camera
        # On step: runs QPLEX executor nội bộ, trả về base obs
    RLlibMultiAgentAPI
...
import mate
from examples.hitmac.wrappers import HiTMACCoordinatorWrapper
...
def make_env(env_config):
    ...
    # DiscreteCamera PHẢI đặt trước MultiCamera (executor cần discrete action space)
    ...
    # Phase 2A: MAPPO coordinator với QPLEX executor (fixed)
...
tune.register_env('mate-hitmac.coordinator.mappo.camera', make_env)

config = {
    'framework': 'torch',
    'seed': 0,
    # === Environment ==============================================================================
    'env': 'mate-hitmac.coordinator.mappo.camera',
    'env_config': {
