...
# Run: python -m examples.hrl_v2.a3c.camera.train
...
from examples.hrl_v2.a3c.camera.config import config
...
# Training resources for A3C
# A3C is designed for asynchronous training with many workers
...
def train(
    ...
    if WandbLoggerCallback.is_available():
        project = project or ('mate-hrl_v2' if not DEBUG else 'mate-hrl_v2-debug')
        group = group or f'hrl_v2.a3c.camera.{experiment.name}'
    ...
    with open(HERE / 'config.txt', 'w', encoding='utf-8') as f:
        f.write(str(experiment.spec['config']))
...
def main():
    ...
    parser.add_argument('--num-envs-per-worker', type=int, metavar='ENV', default=1)
