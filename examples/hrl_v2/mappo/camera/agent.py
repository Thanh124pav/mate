"""
MAPPO Agent for HRL V2 Camera Control.
...
class HRLv2MAPPOCameraAgent(mate.CameraAgentBase):
    """
    MAPPO-based camera agent for HRL V2.
    
    Uses greedy high-level assignment and learned low-level control.
    Multi-Agent PPO with shared parameters across cameras.
    """
    ...
    def __init__(
        ...
        **kwargs
        ...
        if checkpoint_path is not None:
            ...
            self.trainer = PPOTrainer(env='mate-hrl_v2.mappo.camera')
            self.trainer.restore(checkpoint_path)
            self.policy = self.trainer.get_policy(policy_id)
        else:
            raise ValueError("checkpoint_path is required for HRLv2MAPPOCameraAgent")
    ...
    def act(self, observation, info=None):
        ...
        2. Low-level: Use MAPPO policy to control camera
        """
        observation = np.asarray(observation)
        ...
        # Low-level actions from MAPPO policy
        camera_actions = []
        new_lstm_states = {}
