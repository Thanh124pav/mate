import copy

from gym import spaces
from ray import tune
from ray.rllib.agents.qplex_focus import DEFAULT_CONFIG
from ray.rllib.models import MODEL_DEFAULTS

import mate
from examples.utils import (
    CustomMetricCallback,
    FrameSkip,
    RLlibMultiAgentAPI,
    RLlibMultiAgentCentralizedTraining,
)


def target_agent_factory():
    return mate.agents.GreedyTargetAgent(seed=0)


def make_env(env_config):
    env_config = env_config or {}
    env_id = env_config.get('env_id', 'MultiAgentTracking-v0')
    base_env = mate.make(
        env_id, config=env_config.get('config'), **env_config.get('config_overrides', {})
    )
    if str(env_config.get('enhanced_observation', None)).lower() != 'none':
        base_env = mate.EnhancedObservation(base_env, team=env_config['enhanced_observation'])

    discrete_levels = env_config.get('discrete_levels', None)
    assert discrete_levels is not None, 'QPLEX only supports discrete actions.'
    base_env = mate.DiscreteCamera(base_env, levels=discrete_levels)

    target_agent = env_config.get('opponent_agent_factory', target_agent_factory)()
    env = mate.MultiCamera(base_env, target_agent=target_agent)

    env = mate.RelativeCoordinates(env)
    env = mate.RescaledObservation(env)
    env = mate.RepeatedRewardIndividualDone(env)

    if 'reward_coefficients' in env_config:
        env = mate.AuxiliaryCameraRewards(
            env,
            coefficients=env_config['reward_coefficients'],
            reduction=env_config.get('reward_reduction', 'none'),
        )

    frame_skip = env_config.get('frame_skip', 1)
    if frame_skip > 1:
        env = FrameSkip(env, frame_skip=frame_skip)

    env = RLlibMultiAgentAPI(env)
    env = RLlibMultiAgentCentralizedTraining(env)
    action_space = spaces.Tuple((env.action_space,) * len(env.agent_ids))
    observation_space = spaces.Tuple((env.observation_space,) * len(env.agent_ids))
    setattr(observation_space, 'original_space', copy.deepcopy(observation_space))

    env = env.with_agent_groups(
        groups={'camera': env.agent_ids}, obs_space=observation_space, act_space=action_space
    )
    return env


tune.register_env('mate-qplex_focus.camera', make_env)

config = {
    **DEFAULT_CONFIG,
    'framework': 'torch',
    'seed': 0,
    # === Environment ==============================================================================
    'env': 'mate-qplex_focus.camera',
    'env_config': {
        'env_id': 'MultiAgentTracking-v0',
        'config': 'MATE-4v8-9.yaml',
        'config_overrides': {'reward_type': 'dense'},
        'reward_coefficients': {'coverage_rate': 1.0},  # override env's raw reward
        'reward_reduction': 'mean',  # shared reward
        'discrete_levels': 5,
        'frame_skip': 5,
        'enhanced_observation': 'none',
        'opponent_agent_factory': target_agent_factory,
    },
    'disable_env_checking': True,
    'horizon': 500,
    'callbacks': CustomMetricCallback,
    # === Model ====================================================================================
    'normalize_actions': True,
    'model': {
        **MODEL_DEFAULTS,
        'fcnet_hiddens': [512, 256],  # not used
        'fcnet_activation': 'tanh',
        'lstm_cell_size': 384,
        'custom_model_config': {
            'focus_bias_hidden_dim': 512,
            'focus_bias_num_layers': 3,
        },
        'max_seq_len': 10000,  # for complete episode
    },
    'mixer': 'qplex_focus',
    'mixing_embed_dim': 128,
    'focus': {
        'enabled': True,
        'use_env_params': True,
        'alpha_credit': 0.05,
        'beta_belief': 0.1,
        'belief_mode': 'learned',
        'horizon': 3,
        'horizon_discount': 0.7,
        'belief_hidden_dim': 256,
        'belief_arch': 'mlp',
        'belief_num_layers': 1,
        'belief_dropout': 0.0,
        'belief_max_delta': 400.0,
        'belief_min_std': 25.0,
        'integral_mode': 'MC',
        'sigma_method': 'gauss_hermite',
        'sigma_order': 8,
        'sample_chunk_size': 32,
        'mc_num_points': 128,
        'mc_chunk_size': 32,
        'mc_seed': 0,
        'grid_size': 64,
        'grid_chunk_size': 128,
        'grid_x_range': (-1000.0, 1000.0),
        'grid_y_range': (-1000.0, 1000.0),
        'use_action_selection': False,
        'min_credit_signal': 1e-6,
        'use_signal_confidence': True,
        'signal_weight_min': 0.1,
        'signal_weight_max': 3.0,
        'action_bias_eta': 2.0,
        'action_bias_bootstrap': False,
        'action_bias_confidence_enabled': True,
        'action_bias_confidence_std_threshold': 250.0,
        'action_bias_confidence_std_temperature': 100.0,
        'action_bias_confidence_min': 0.1,
        'action_bias_confidence_max': 1.0,
        'action_distill_enabled': True,
        'action_distill_coeff': 15.0,
        'action_distill_mode': 'soft_kl',
        'action_distill_policy_q_enabled': True,
        'action_distill_policy_q_coeff': 4.0,
        'action_distill_bias_coeff': 2.0,
        'local_belief_enabled': True,
        'local_belief_coeff': 5.0,
        'local_belief_nll_coeff': 0.1,
        'local_belief_mse_coeff': 20.0,
        'local_belief_visible_nll_coeff': 0.1,
        'local_belief_visible_mse_coeff': 80.0,
        'local_belief_hidden_dim': 512,
        'local_belief_num_layers': 3,
        'local_belief_dropout': 0.0,
        'local_belief_min_std': 0.01,
        'local_belief_action_guide_enabled': True,
        'local_belief_action_eta': 10.0,
        'local_belief_action_distill_coeff': 2.0,
        'local_action_student_distill_coeff': 4.0,
        'local_belief_use_visible_obs': True,
        'teacher_temperature': 1.0,
        'student_temperature': 1.2,
        'action_distill_confidence': 'off',
        'student_action_bias_enabled': True,
        'student_action_bias_eta': 0.0,
        'student_action_bias_bootstrap': True,
        'action_bias_rotation_step': 5.0,
        'action_bias_zooming_step': 2.5,
        'action_bias_min_viewing_angle': 30.0,
        'action_bias_max_viewing_angle': 180.0,
        'action_bias_max_sight_range': 1500.0,
        'action_bias_real_only': True,
        'action_bias_clip': 3.0,
        'action_bias_angular_temp': 0.18,
        'action_bias_range_temp': 150.0,
        'eps': 1e-8,
    },
    # === Policy ===================================================================================
    'gamma': 0.99,
    # === Exploration ==============================================================================
    'explore': True,
    'exploration_config': {
        'type': 'EpsilonGreedy',
        'initial_epsilon': 1.0,
        'final_epsilon': 0.02,
        'epsilon_timesteps': 50000,  # trained environment steps
    },
    # === Replay Buffer & Optimization =============================================================
    'batch_mode': 'complete_episodes',
    'rollout_fragment_length': 0,  # send sampled episodes to the replay buffer immediately
    'buffer_size': 500,  # reduced from 2000 to fit in memory (each item contains `num_workers` episodes)
    'timesteps_per_iteration': 5120,  # environment steps
    'learning_starts': 5000,  # environment steps
    'train_batch_size': 1024,  # environment steps
    'target_network_update_freq': 50,  # environment steps
    'metrics_num_episodes_for_smoothing': 25,
    'grad_norm_clipping': 1000.0,
    'lr': 1e-4,
}
