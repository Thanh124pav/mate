...
.. code:: bash
    ...
    python3 -m examples.mappo_focus.camera

    python3 -m mate.evaluate --episodes 1 --render-communication \
        --camera-agent examples.mappo_focus:MAPPOFocusCameraAgent \
        --camera-kwargs '{ "checkpoint_path": "examples/mappo_focus/camera/ray_results/MAPPO_FOCUS/latest-checkpoint" }'
"""
...
from examples.mappo_focus.camera.train import experiment
...
def main():
    ...
    if not os.path.exists(args.checkpoint_path):
        print(
            (
                ...
                f'  python -m examples.mappo_focus.camera.train'
            ),
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    # Make agents ##############################################################
    camera_agent = MAPPOFocusCameraAgent(checkpoint_path=args.checkpoint_path)
