...
r"""
Example of hierarchical QMIX+FOCUS agents for the Multi-Agent Tracking Environment.

.. code:: bash

    python3 -m examples.hrl.qmix_focus.camera

    python3 -m mate.evaluate --episodes 1 --render-communication \
        --camera-agent examples.hrl.qmix_focus.camera:CameraAgent \
        --camera-kwargs '{ "checkpoint_path": "examples/hrl/qmix_focus/camera/ray_results/HRL-QMIX-FOCUS/latest-checkpoint" }'
"""
...
import mate
from examples.hrl.qmix_focus.camera.agent import HRLQMIXFocusCameraAgent
from examples.hrl.qmix_focus.camera.train import experiment
    ...
    if not os.path.exists(args.checkpoint_path):
        print(
            (
                ...
                f'  python -m examples.hrl.qmix_focus.camera.train'
            ),
            file=sys.stderr,
        )
        sys.exit(1)
