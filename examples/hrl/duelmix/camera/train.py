...
# Run: python -m examples.hrl.duelmix.camera.train
...
from examples.hrl.duelmix.camera.config import config
...
def train(
    ...
    if WandbLoggerCallback.is_available():
        ...
        group = group or f'hrl.duelmix.camera.{experiment.name}'
    ...
    with open('examples/hrl/duelmix/config.txt', 'w', encoding='utf-8') as f:
