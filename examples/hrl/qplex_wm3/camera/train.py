...
# Run: python -m examples.hrl.qplex_wm3.camera.train --env MATE-4v8-9.yaml
...
import mate
from examples.hrl.qplex_wm3.camera.config import config
...
def train(experiment, project=None, group=None, local_dir=None,
          num_gpus=NUM_GPUS_FOR_TRAINER, num_workers=NUM_WORKERS,
          num_envs_per_worker=8, seed=None, timesteps_total=None,
          buffer_capacity=2000, restore=None, resume=False, env=None):
    if env is not None:
        experiment.spec['config']['env_config']['config'] = env
        tmp_env = mate.make('MultiAgentTracking-v0', config=env)
        n_targets = tmp_env.num_targets
        tmp_env.close()
        experiment.spec['config']['wm3']['n_targets'] = n_targets
    ...
    if WandbLoggerCallback.is_available():
        ...
        group = group or f'hrl.qplex_wm3.camera.{experiment.name}'
    ...
    experiment.spec['config'].update(
        num_cpus_for_driver=NUM_CPUS_FOR_TRAINER, num_gpus=num_gpus,
        num_gpus_per_worker=0, num_workers=num_workers,
    ...
    return tune.run(experiment, metric='episode_reward_mean', mode='max',
                    callbacks=tune_callbacks, verbose=3, restore=restore, resume=resume)
...
def main():
    ...
    return train(experiment, **vars(args))
