...
from ray.rllib.agents.qplex_se.qplex_policy import QPLEXSETorchPolicy

import mate
from examples.hrl.qplex_se.camera.config import config as _config
from examples.hrl.qplex_se.camera.config import make_env as _make_env
...
class HRLQPLEXSE_CameraAgent(RLlibGroupedPolicyMixIn, mate.CameraAgentBase):
    """Hierarchical QPLEX + Shared Encoder Camera Agent.

    A wrapper for the trained RLlib policy with shared encoder augmentation.
    The shared encoder learns cross-agent representations during training,
    but at inference time only the Q-network is used for action selection.
    """

    POLICY_CLASS = QPLEXSETorchPolicy
