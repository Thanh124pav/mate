...
# Run: python -m examples.hrl.qplex_se.camera.train
...
import mate
from examples.hrl.qplex_se.camera.config import config
...
def train(
    ...
    if env is not None:
        ...
        tmp_env.close()
        experiment.spec['config']['shared_encoder']['n_targets'] = n_targets
    ...
    if WandbLoggerCallback.is_available():
        ...
        group = group or f'hrl.qplex_se.camera.{experiment.name}'
...
def main():
    ...
    parser.add_argument(
        '--local-dir', type=str, metavar='DIR', default=LOCAL_DIR,
        help='Local directory for the experiment (default: %(default)s)',
    )
    parser.add_argument(
        '--num-gpus', type=float, metavar='GPU', default=NUM_GPUS_FOR_TRAINER,
        help='number of GPUs for trainer (default: %(default)g)',
    )
    parser.add_argument(
        '--num-workers', type=int, metavar='WORKER', default=NUM_WORKERS,
        help='number of rollout workers (default: %(default)d)',
    )
    parser.add_argument(
        '--num-envs-per-worker', type=int, metavar='ENV', default=8,
        help='number of environments per rollout worker (default: %(default)d)',
    )
    parser.add_argument(
        '--timesteps-total', type=float, metavar='STEP', default=10e6,
    ...
    parser.add_argument(
        '--buffer-capacity', type=float, metavar='EPISODE', default=100,
        help='capacity for the replay buffer (default: %(default).1e episodes)',
    )
    parser.add_argument(
        '--restore', type=str, metavar='PATH', default=None,
    ...
    parser.add_argument(
        '--env', default=None,
        help='.yaml environment config (auto-detects n_targets)',
    )
