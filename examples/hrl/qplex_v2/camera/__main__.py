...
r"""
Example of hierarchical QPLEXV2 agents for the Multi-Agent Tracking Environment.

.. code:: bash

    python3 -m examples.hrl.qplex.camera

    python3 -m mate.evaluate --episodes 1 --render-communication \
        --camera-agent examples.hrl:HRLQPLEXV2CameraAgent \
        --camera-kwargs '{ "checkpoint_path": "examples/hrl/qplex/camera/ray_results/HRL-QPLEX/latest-checkpoint" }'
"""
...
from examples.hrl.qplex_v2.camera.train import experiment
...
def main():
    ...
    if not os.path.exists(args.checkpoint_path):
        print(
            (
                ...
                f'  python -m examples.hrl.qplex.camera.train'
            ),
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    # Make agents ##############################################################
    camera_agent = HRLQPLEXV2CameraAgent(checkpoint_path=args.checkpoint_path)
