...
# Run: python -m examples.hrl.qplex_focus_v2.camera.train
...
from examples.hrl.qplex_focus_v2.camera.config import config
...
def train(
    ...
    if WandbLoggerCallback.is_available():
        ...
        group = group or f'hrl.qplex_focus_v2.camera.{experiment.name}'
    ...
    if restore is not None:
        # tune.run ignores `restore` when passed an Experiment object (Ray 1.x bug):
        # it only applies restore when constructing a new Experiment internally.
        experiment.spec['restore'] = restore
    ...
    with open('examples/hrl/qplex_focus_v2/config.txt', 'w', encoding='utf-8') as f:
...
def main():
    ...
    parser.add_argument(
        '--env',
        default=None,
        help='.yaml environment to run experiments'
    )
