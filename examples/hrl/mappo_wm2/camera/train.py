...
# Run: python3 -m examples.hrl.mappo_wm2.camera.train
...
from examples.hrl.mappo_wm2.camera.config import config
...
def train(
    ...
    if WandbLoggerCallback.is_available() and os.getenv('WANDB_DISABLED', '').lower() not in ('1', 'true', 'yes', 'on') and os.getenv('WANDB_MODE', '').lower() != 'disabled':
        ...
        group = group or f'hrl.mappo_wm2.camera.{experiment.name}'
