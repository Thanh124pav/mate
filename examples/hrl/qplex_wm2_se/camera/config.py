"""HRL QPLEX_WM2_SE — RSSM World Model + Shared Encoder + HierarchicalCamera.
...
def target_agent_factory():
    """Training: evasive targets (avoid cameras) → camera action affects target movement."""
    return EvasiveTargetAgent(seed=0, avoidance_strength=0.5, avoidance_range=0.5)

def greedy_target_factory():
    """Evaluation: standard greedy targets (no avoidance)."""
...
def make_env(env_config):
    ...
    if 'reward_coefficients' in env_config:
        env = mate.AuxiliaryCameraRewards(
            env, coefficients=env_config['reward_coefficients'],
...
tune.register_env('mate-hrl.qplex_wm2_se.camera', make_env)
...
config = {
    **DEFAULT_CONFIG,
    'framework': 'torch',
    'seed': 0,
    'env': 'mate-hrl.qplex_wm2_se.camera',
    'env_config': {
