...
import mate
from examples.hrl.qplex_wm2_se.camera.config import config as _config
from examples.hrl.qplex_wm2_se.camera.config import make_env as _make_env
...
class HRLQPLEXWM2SE_CameraAgent(RLlibGroupedPolicyMixIn, mate.CameraAgentBase):
    """HRL QPLEX_WM2_SE Camera Agent — RSSM World Model + SE + HierarchicalCamera.

    WM2 latent features augment obs/state; SE position-prediction auxiliary
    loss operates on RNN hidden states. At inference only Q-network is used.
    """
    ...
    def act(self, observation, info=None, deterministic=None):
        ...
        self.last_action = HierarchicalCamera.executor(
            self.state, target_states,
            target_selection_bits=self.last_selection, target_view_mask=tracked_bits,
        )
