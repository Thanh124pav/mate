...
# Run: python -m examples.hrl.duelmix_wm2.camera.train --env MATE-4v8-9.yaml
...
import mate
from examples.hrl.duelmix_wm2.camera.config import config
...
def train(experiment, project=None, group=None, local_dir=None,
          ...
          num_envs_per_worker=8, target_agent=None, evaluation_interval=5, seed=None, timesteps_total=None,
    ...
    if WandbLoggerCallback.is_available() and os.getenv('WANDB_DISABLED', '').lower() not in ('1', 'true', 'yes', 'on') and os.getenv('WANDB_MODE', '').lower() != 'disabled':
        ...
        group = group or f'hrl.duelmix_wm2.camera.{experiment.name}'
...
def main():
    ...
    parser.add_argument('--buffer-capacity', type=float, metavar='EPISODE', default=100)
    parser.add_argument('--restore', type=str, metavar='PATH', default=None)
